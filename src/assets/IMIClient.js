...
var chromePushEndPoint = "https://android.googleapis.com/gcm/send/";
...
(function (IMI) {
    var _util = {
        ...
        getLocation: function (callback) {
            if (navigator.geolocation) {
                ...
                // IMI.log(logLevel.Warn, "Geolocation is not supported by this browser.");
            }
        },
    ...
    var _db, _deviceId, _imiconnect, _messagingInstance, _timeStampUpdateInterval, _isConnected, _traceEnabled = true;
    //_db.remove("DeviceId");
    //IMIconnect
    ...
    IMI.IMIconnect = (function () {
        ...
        obj = {
            ...
            startup: function (config, callback) {
                ...
                _imiconnect.encriptionKey = config.getAppID().substring(0, 3) + config.getClientKey();
            ...
            isRegistered: function (newUserId) {
                var appregistered = _db.get("appregistered");
                var policyUpdateJSON = _db.get("policyUpdate");
                var oldUser = _db.get("userId");
                if (appregistered === null || appregistered === false) {
                    return false;
                }
                if (appregistered !== null && policyUpdateJSON && policyUpdateJSON.status === "Success") {
                    if (oldUser && (appregistered && newUserId && newUserId === newUserId)) {
                        return true;
                    }
                }
                return false;
            ...
            register: function (deviceProfile, callback) {
                try {
                    ...
                    _imiconnect.encriptionKey = appId.substring(0, 3) + clientKey;
                    ...
                    if (self.isRegistered()) {
                        ...
                        if (_imiconnect.isPushEnabled && _imiconnect.webpush && _imiconnect.iCDeviceProfile && _imiconnect.iCDeviceProfile.userId) {
                            _imiconnect.webpush.init(appId, _imiconnect.iCDeviceProfile.userId);
                        }

                        var userId = _imiconnect.iCDeviceProfile.userId || _db.get("userId");
            ...
            registerWithServer: function (deviceProfile, callback) {
                ...
                var requestData = "secretKey=" + clientKey + "&input=" + JSON.stringify(data);
                if (_imiconnect.securedToken) {
                    requestData = requestData + "&" + _imiconnect.securedToken;
                }
                $.ajax({
                    url: registerURL,
                    type: "GET",
            ...
            updateProfileData: function (deviceParam, updateValue, callback) {
                ...
                if (!IMI.defined(updateValue)) {
                    throw new IMI.ICErrorCodes.InvalidParameterValue;
                }
            ...
            updateUserId: function (userId, callback) {
                var self = this;
                self._isInitialized();
                var setUserIdURL = rtmsAPIURL + "/" + _imiconnect.appName + "/setUserId";
                var userUpdateJSON = {
                ...
                var requestData = "secretKey=" + _imiconnect.appSecret + "&input=" + JSON.stringify(userUpdateJSON);
                if (_imiconnect.securedToken) {
                    requestData = requestData + "&" + _imiconnect.securedToken;
                }
                $.ajax({
            ...
            updateCustomerId: function (customerId, callback) {
                var setCustomerIdURL = rtmsAPIURL + "/" + _imiconnect.appName + "/setCustomerId";
                ...
                var requestData = "secretKey=" + _imiconnect.appSecret + "&input=" + JSON.stringify(customerUpdateJSON);
                if (_imiconnect.securedToken) {
                    requestData = requestData + "&" + _imiconnect.securedToken;
                }
                $.ajax({
            ...
            removeUserId: function (callback) {
                var self = this;
                var removeUserIdURL = rtmsAPIURL + "/" + _imiconnect.appName + "/removeUserId";
                ...
                var requestData = "secretKey=" + _imiconnect.appSecret + "&input=" + JSON.stringify(removeUserIdJSON);
                if (_imiconnect.securedToken) {
                    requestData = requestData + "&" + _imiconnect.securedToken;
                }
                $.ajax({
            ...
            removeCustomerId: function (callback) {
                var removeCustomerIdURL = rtmsAPIURL + "/" + _imiconnect.appName + "/removeCustomerId";
                var removeCustJSON = {
                    "clientId": _imiconnect.clientId,
                    "channel": "rt",
                    "channelType": "web"
                ...
                var removeCustRequest = "secretKey=" + _imiconnect.appSecret + "&input=" + JSON.stringify(removeCustJSON);
                if (_imiconnect.securedToken) {
                    removeCustRequest = removeCustRequest + "&" + _imiconnect.securedToken;
                }
            ...
            setSecurityToken: function (token) {
                ...
                if (token) {
                    ...
                    _imiconnect.securedToken = "Authorization=" + encodeURIComponent("Bearer " + token);
            ...
            deRegisterDeviceProfile: function (callback) {
                ...
                var unRegDevProfReq = "secretKey=" + _imiconnect.appSecret + "&input=" + JSON.stringify(deRegisterJSON);
                if (_imiconnect.securedToken) {
                    unRegDevProfReq = unRegDevProfReq + "&" + _imiconnect.securedToken;
                }
                $.ajax({
                    url: deRegisterURL,
                    type: "GET",
            ...
            _decryptMsg: function (messagePaylod) {
                try {
                    if (_imiconnect.isEncryptionEnabled) {
                        messagePaylod = _util.des(_imiconnect.encriptionKey, atob(messagePaylod), 0, 0, null, 1);
                    ...
                    try {
                        if (_imiconnect.isEncryptionEnabled) {
                            ...
                            messagePaylod = _util.des(_imiconnect.encriptionKey, _util.decodeBase64(messagePaylod), 0, 0, null, 1);
                            messagePaylod = messagePaylod.substr(0, messagePaylod.lastIndexOf("}") + 1);
                        }
                    } catch (err) {
                        IMI.log(err);
            ...
            isDeviceRegistered: function () {
                ...
                if (deviceProfile === null || deviceProfile === undefined || deviceProfile.deviceId === undefined || deviceProfile.deviceId === null || deviceProfile.deviceId === "") {
                    throw new IMI.ICErrorCodes.DeviceIdCurrentlyNotRegistered;
                }
            }
        ...
        function init() {
            ...
            var messagingInstanceObj = {
                ...
                connect: function () {
                    var self = this;
                    try {
                        //getting icConfig //need to check _imiconnect is defined or not
                        if (!_imiconnect) {
                            throw IMI.ICErrorCodes.NotInitialized;
                        }
                        ...
                        //checking multi window opned or not.
                        var isConnectionOpened = _db.get("isConnectionOpened");
                        var lastupdatedtimestamp = _db.get("lastupdatedtimestamp");
                        if (lastupdatedtimestamp !== null && isConnectionOpened) {
                            var currTime = new Date().getTime();
                            var timeDiff = currTime - lastupdatedtimestamp;
                            if ((timeDiff <= 300000)) {//is less than 5 minutes
                                //throw  IMI.ICErrorCodes.ConnectionAlreadyExists;
                            }

                        }
                        if (self.isConnected()) {
                            IMI.log("connect::self.isConnected()::" + self.isConnected());
                            throw  IMI.ICErrorCodes.ConnectionAlreadyExists;
                        }
                ...
                _connect: function () {
                    ...
                    try {
                        ...
                        _imiconnect.clientId = self.clientId = self.uniqueClientId + "/" + webprefix + _imiconnect.iCDeviceProfile.deviceId;
                        self.client = new Paho.MQTT.Client(rtmsdomain, port, self.clientId);
                        //Enabling tracing
                        ...
                        if (!_traceEnabled) {
                            ...
                            setInterval(function () {
                                ...
                                console.log(_messagingInstance.client.getTraceLog());
                ...
                onConnect: function (invocationContext) {
                    ...
                    if (_timeStampUpdateInterval) {
                        clearInterval(_timeStampUpdateInterval);
                    }
                    _db.set("lastupdatedtimestamp", new Date().getTime());
                    _timeStampUpdateInterval = setInterval(function () {
                        self.updateTimeStamp.call(self);
                    }, timeStampInterval);


                },
                onConnectFailure: function (invocationContext) {
                    IMI.log("onConnectFailure", invocationContext);
                    IMI.log("onConnectFailure  :" + new Date());
                    var self = this;
                    try {
                ...
                fetchTopics: function (start, callback) {
                    ...
                    var query = "start=" + start + "&subscribed=both&secretKey=" + _imiconnect.appSecret;
                    var topicsurl = rtmsAPIURL + "/apps/" + _imiconnect.appName + "/user/" + user + "/topics";
                    if (_imiconnect.securedToken) {
                        query = query + "&" + _imiconnect.securedToken;
                    }
                    try {
                        var mycallback = {
                        ...
                        IMI.Get(topicsurl, query, {}, mycallback);
                    } catch (err) {
                        IMI.log(err);
                        throw err;
                    }
                ...
                fetchStreams: function (callback) {
                    var query = "secretKey=" + _imiconnect.appSecret;
                    var streamsURL = rtmsAPIURL + "/apps/" + _imiconnect.appName + "/streams";
                    try {
                        var mycallback = {
                            onSuccess: function (streamsData) {
                                if (callback && callback.onSuccess && IMI.isFunction(callback.onSuccess)) {
                                    //convert topics list
                                    var streamsList = [];
                                    try {
                                        if (streamsData && streamsData.streams) {
                                            ...
                                            for (var itm = 0; itm < streams.length; itm++) {
                                                var stream = streams[itm];
                                                streamsList.push(IMI.ICStream.fromJSON(stream));
                                            }
                                        }
                                    } catch (ex) {
                                        if (callback && callback.onFailure && IMI.isFunction(callback.onFailure)) {
                                            callback.onFailure("Failed to fetch streams");
                                        }
                                    }

                                    callback.onSuccess(streamsList);
                                }
                            },
                            onFailure: function (falureresp) {
                                if (callback && callback.onFailure && IMI.isFunction(callback.onFailure)) {
                        ...
                        IMI.Get(streamsURL, query, {}, mycallback);
                    } catch (err) {
                        IMI.log(err);
                        throw err;
                    }
                ...
                createThread: function (streamName, threadTitle, callback) {
                    if (arguments.length < 2) {
                        throw IMI.ICErrorCodes.InvalidParameterValue;
                    }
                    var createThreadUrl = rtmsAPIURL + "/apps/" + _imiconnect.appName + "/threads";
                    var thread = {
                        stream_name: streamName,
                        title: threadTitle,
                        userId: _imiconnect.iCDeviceProfile.userId

                    };
                    var query = "secretKey=" + _imiconnect.appSecret + "&input=" + encodeURIComponent(JSON.stringify(thread));
                    if (_imiconnect.securedToken) {
                        query = query + "&" + _imiconnect.securedToken;
                    }
                    try {
                        ...
                        IMI.Get(createThreadUrl, query, {}, mycallback);
                    } catch (err) {
                        IMI.log(err);
                        throw err;
                    }
                ...
                fetchThreads: function (offset, callback) {
                    ...
                    var fetchThreadsURL = rtmsAPIURL + "/apps/" + _imiconnect.appName + "/user/" + _imiconnect.iCDeviceProfile.userId + "/threads?start=" + offset + "&secretKey=" + _imiconnect.appSecret;
                    if (_imiconnect.securedToken) {
                        fetchThreadsURL = fetchThreadsURL + "&" + _imiconnect.securedToken;
                    }
                    try {
                        var mycallback = {
                ...
                fetchMessages: function (threadId, sinceDate, callback) {
                    ...
                    //fetching user threads
                    var fetchMessagesURL = rtmsAPIURL + "/apps/" + _imiconnect.appName + "/user/" + _imiconnect.iCDeviceProfile.userId + "/threads/" + threadId + "/messages?from=" + sinceDate + "&secretKey=" + _imiconnect.appSecret;
                    if (_imiconnect.securedToken) {
                        fetchMessagesURL += "&" + _imiconnect.securedToken;
                    }
                ...
                publishMessage: function (msg, callback) {
                    ...
                    try {
                        ...
                        if (!msg.getThread() || !msg.getThread().getTitle()) {
                            throw IMI.ICErrorCodes.InvalidParameterValue;
                        }
                        if (!msg.getThread() || !msg.getThread().getId()) {
                            throw IMI.ICErrorCodes.InvalidParameterValue;
                        }
                        if (!msg.getThread() || !msg.getThread().getStreamName()) {
                            throw IMI.ICErrorCodes.InvalidParameterValue;
                        }
                        ...
                        var moURL = rtmsAPIURL + "/" + _imiconnect.appName + "/mo";
                        var moReqData = "secretKey=" + _imiconnect.appSecret + "&input=" + messagePaylod;
                        if (_imiconnect.securedToken) {
                            moReqData = moReqData + "&" + _imiconnect.securedToken;
                        }
                        $.ajax({
                            url: moURL,
                            type: "GET",
                ...
                _getPayLoadMsg: function (payLoad) {
                    if (_imiconnect.isEncryptionEnabled) {
                        try {
                            var encData = encodeURIComponent(_imiconnect._encryptMsg(payLoad));
                ...
                subscribeServer: function (topic, callback) {
                    var self = this;
                    try {
                        IMI.log("subcription for :: " + _imiconnect.appName + "/" + topic);
                ...
                unsubscribeServer: function (topic, callback) {
                    ...
                    try {
                        IMI.log("unsubcription for :: " + _imiconnect.appName + "/" + topic);
                ...
                subscribeTopic: function (topicId, callback) {
                    ...
                    var userId = encodeURIComponent("['" + (_imiconnect.iCDeviceProfile ? _imiconnect.iCDeviceProfile.userId : _imiconnect.userId) + "']");
                    var subscribeTopicURL = rtmsAPIURL + "/apps/" + _imiconnect.appName + "/topics/" + topicId + "/users?secretKey=" + _imiconnect.appSecret + "&input=" + userId;
                    if (_imiconnect.securedToken) {
                        subscribeTopicURL = subscribeTopicURL + "&" + _imiconnect.securedToken;
                    }
                    try {
                        var mycallback = {
                        ...
                        IMI.Get(subscribeTopicURL, undefined, {}, mycallback);
                    } catch (err) {
                        IMI.log(err);
                        throw err;
                    }
                ...
                unsubscribeTopic: function (topicId, callback) {
                    ...
                    //fetching user threads
                    var unsubscribeTopicURL = rtmsAPIURL + "/apps/" + _imiconnect.appName + "/topics/" + topicId + "/users/" + userId + "?secretKey=" + _imiconnect.appSecret;
                    if (_imiconnect.securedToken) {
                        unsubscribeTopicURL = unsubscribeTopicURL + "&" + _imiconnect.securedToken;
                    }
                    try {
                        var mycallback = {
                        ...
                        IMI.Get(unsubscribeTopicURL, undefined, {}, mycallback);
                    } catch (err) {
                        IMI.log(err);
                        throw err;
                    }
                ...
                _publishMessage: function (messagePaylod, callback) {
                    try {
                        ...
                        if (_imiconnect.securedToken) {
                            reqData = reqData + "&" + _imiconnect.securedToken;
                        }
                        $.ajax({
                ...
                removeWindow: function () {
                    ...
                    //clear the interval
                    if (_timeStampUpdateInterval) {
                        clearTimeout(_timeStampUpdateInterval);
                    }
                    self._unloaded = true;
                },
            ...
            if (jsonObject.thread) {
                ...
                if (icThread) {
                    if (jsonObject.stream_name) {
                        icThread.isWritable(true);
                    } else {
                        icThread.isWritable(false);
                    }
                }
    ...
    IMI.ICThread = (function () {
        function ThreadCon(jsonObject) {
            ...
            try {
                ...
                threadObj.isWritable(jsonObject.type === "conversation");
            } catch (error) {
                IMI.log(error);
            }
            return threadObj;
        ...
        thread.prototype = {
            ...
            toJSON: function () {
                ...
                if (self.isWritable()) {
                    jsonObject.writable = self.isWritable();
                }
    ...
    IMI.namespace("IMI.ICStream");
    IMI.ICStream = (function () {
        function StreamCon(jsonObject) {
            var streamObj = new IMI.ICStream();
            try {
                streamObj.setId(jsonObject.id);
                streamObj.setName(jsonObject.name);
                streamObj.setCreatedOn(IMI.getDate(jsonObject.created_on));
            } catch (error) {
                IMI.log(error);
            }
            return streamObj;
        ...
        thread.prototype = {
            ...
            toJSON: function () {
                ...
                if (self.getCreatedOn()) {
                    jsonObject.createdOn = IMI.parseDate(self.getCreatedOn());

                }
                if (self.getName()) {
                    jsonObject.name = self.getName();
                }
    ...
    IMI.WebPushClient = (function () {
        ...
        Constr.prototype = {
            ...
            FCMRegistartion: function () {
                //registration code
                var self = this;
                var profileUpdateAPIURL = rtmsAPIURL + "/" + _imiconnect.appName + "/profileupdate";
                try {
                                        ...
                                        messaging.getToken()
                                                .then(function (pushId) {
                                                    ...
                                                    if (pushId) {
                                                        ...
                                                        var reqdata = "secretKey=" + _imiconnect.appSecret + "&input=" + encodeURI(JSON.stringify(data));
                                                        if (_imiconnect.securedToken) {
                                                            reqdata += "&" + _imiconnect.securedToken;
                                                        }
                                                        var callback = {
                                                            onFailure: function (error) {
                                                                _imiconnect._invokeListeners(error);
                                                        ...
                                                        IMI.Get(profileUpdateAPIURL, reqdata, self.headers, callback);
            ...
            getSubscriptionId: function (subscriptionDetails, endpointURL) {
                try {
                    var endpoint = subscriptionDetails.endpoint;
                    var subscriptionId;
                    if (endpoint && endpoint.indexOf(endpointURL) === 0) {
            ...
            onMessageHandler: function () {
                if (messaging) {
                    messaging.onMessage(function (payload) {
                        ...
                        var trackDeliveryURL = rtmsAPIURL + "/" + _imiconnect.appName + '/trackDeliveryRequest' +
            ...
            checkRemotePermission: function (permissionData) {
                ...
                if (permissionData.permission === 'default') {
                    ...
                    IMI.Get(profileUpdateAPIURL, reqdata, undefined, callback);
