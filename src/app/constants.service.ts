...
export class ConstantsService {
  ...
  readonly HANDSON_TABLE_KNOWLEDGE_BASE_SETTING = {
    cells: function (row, col) {
      ...
      if (row === 0) {
        cellProperties['renderer'] = function (instance, td, row_temp, col_temp, prop, value, cellProperties_temp) {
  ...
  getConnectLoginUrl() {
    return this.BACKEND_URL + 'api/v1/user/connectLogin/';
  }
  ...
  getReportUrl(limit = 1, offset = 10) {
    return this.BACKEND_URL + `api/v1/reports/?limit=${limit}&offset=${offset}&order_by=-created_at`;
  }

  getReportHistoryUrl(limit = 1, offset = 10, order_by?) {
    return this.BACKEND_URL + `api/v1/reporthistory/?limit=${limit}&offset=${offset}&order_by=-created_at`;
  }
  ...
  getAllVersionsByBotId() {
    return this.BACKEND_URL + 'api/v1/botversioning/?limit=1000';
  }
