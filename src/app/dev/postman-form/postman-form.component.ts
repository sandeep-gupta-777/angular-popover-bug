...
export class PostmanFormComponent {
  ...
  makeHttpReq() {
    this.loading = true;
    let formData = this.postManForm.value;
    let functionToCall, body, headerData;

    if(this.postManForm.get('method').value === EHttpVerb.GET){functionToCall='makeGetReq'}
    if(this.postManForm.get('method').value === EHttpVerb.POST){functionToCall='makePostReq'}
    if(this.postManForm.get('method').value === EHttpVerb.PUT){functionToCall='makePutReq'}
    if(this.postManForm.get('method').value === EHttpVerb.DELETE){functionToCall='makeDeleteReq'}
